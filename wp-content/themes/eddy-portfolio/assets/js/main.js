...
$(function () {
  ...
  // ── 10. Formulaire de contact — AJAX WordPress ──
  $('#contact-form').on('submit', function (e) {
    ...
    if (!name)                         { showError('name-error', 'contact-name');       valid = false; }
    if (!email || !isValidEmail(email)) { showError('email-error', 'contact-email');     valid = false; }
    if (!subject)                       { showError('subject-error', 'contact-subject'); valid = false; }
    if (message.length < 10)            { showError('message-error', 'contact-message'); valid = false; }

    if (valid) submitContactForm(name, email, subject, message);
  });
  ...
  function submitContactForm(name, email, subject, message) {
    ...
    $btn.prop('disabled', true).html('<span class="spinner"></span> Envoi...');

    var ajaxUrl = (typeof EDDY_VARS !== 'undefined') ? EDDY_VARS.ajax_url : '/wp-admin/admin-ajax.php';
    var nonce   = (typeof EDDY_VARS !== 'undefined') ? EDDY_VARS.nonce   : '';
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response.success) {
          var successMsg = (typeof EDDY_VARS !== 'undefined') ? EDDY_VARS.i18n.send_success : 'Message envoyé !';
          $btn.html('✓ ' + successMsg).css('background', '#059669');
          $('#form-success').addClass('show');
          $('#contact-form')[0].reset();
          setTimeout(function () {
            $btn.prop('disabled', false).html('Envoyer le message').css('background', '');
            $('#form-success').removeClass('show');
          }, 5000);
        } else {
          var errMsg = (typeof EDDY_VARS !== 'undefined') ? EDDY_VARS.i18n.send_error : 'Erreur lors de l\'envoi.';
          $btn.prop('disabled', false).html('Envoyer le message');
          alert(errMsg);
        }
      },
      error: function () {
  ...
  // ── 11. Copier le lien (page article) ──
