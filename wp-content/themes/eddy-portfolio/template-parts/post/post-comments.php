...
// Charger les commentaires manuellement (get_template_part ne peuple pas $wp_query->comments)
$eddy_comments = get_comments( [
    'post_id' => get_the_ID(),
    'status'  => 'approve',
    'order'   => 'ASC',
] );
$eddy_comment_count = count( $eddy_comments );

if ( ! comments_open() && $eddy_comment_count === 0 ) return;
?>
<div id="comments" class="mt-12">

    <?php if ( $eddy_comment_count > 0 ) : ?>

    <h3 class="text-xl font-bold mb-6" style="color:var(--color-text)">
        <?php
        printf(
            /* translators: %s : nombre de commentaires */
            esc_html( _n( '%s commentaire', '%s commentaires', $eddy_comment_count, 'eddy-portfolio' ) ),
            esc_html( number_format_i18n( $eddy_comment_count ) )
        );
        ?>
    </h3>
